...
def add_company_to_valuation_list(ordered_dict: dict, exchange: str, company_code: str, industry: str) -> None:
	...
	keys_cant_be_negative = ['P/S', 'EV/EBITDA', 'P/TB', 'P/B', 'Debt/Equity', 'Trailing P/E', 'Forward P/E',
							 'P/CFO', 'P/FCF', 'P/NCF', 'P/Div', 'P/Cash', 'P/NCash']
	try:
		with open(file_path, 'r') as json_file:
			...
			if existing_entry:
				...
				for key, value in ordered_dict.items():
					if value is not None and isinstance(value, (int, float)) and not np.isnan(value) and not (
						key in keys_cant_be_negative and value < 0):
		...
		with open(file_path, "r") as json_file:
			...
			for company in json_data['Companies']:
				for key, value in company.items():
					if isinstance(value, float) and not np.isnan(value) and key in numeric_data:
			...
			filtered_data = {
				key: [item for item in numeric_data[key] if isinstance(item, (int, float)) and not np.isnan(item)] for
				key in numeric_data if numeric_data[key]}

			median_values = {key: round(np.median(filtered_data[key]), 2) for key in filtered_data}
			mad_values = {key: round(calculate_median_absolute_deviation(np.array(filtered_data[key])), 2) for key in
						  filtered_data}
